...
def morph_baby(src_img_filename, dst_img_filename, src_points, dst_points, step, result_folder):
    src_img = Image.open(src_img_filename)
    dst_img = Image.open(dst_img_filename)
    ...
    while True:
        ...
        print 'idx_map took ', time.time()-time_a
        ...
        print 'get now_points ', time.time()-time_a

        now_triangles = ids_2_triangles(triangle_ids, now_points)

        print 'get now_triangles ', time.time()-time_a
        ...
        for h in range(output_height):
            for w in range(output_width):
                ...
                '''
                v0 = int(dst_pixel[0]*phase+src_pixel[0]*(1-phase))
                v1 = int(dst_pixel[1]*phase+src_pixel[1]*(1-phase))
                v2 = int(dst_pixel[2]*phase+src_pixel[2]*(1-phase))
        ...
        for t in now_triangles:
            ...
            draw_line(tmp_pixels, output_size, (255,0,0), t[0], t[2])
            draw_line(tmp_pixels, output_size, (255,0,0), t[1], t[2])
    ...
    writeGif(store_path, img_list, duration=0.1)
        
    print 'time cost:', time.time()-time_a0
