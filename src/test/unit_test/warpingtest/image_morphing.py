...
def morph_baby(src_img, dst_img, src_points, dst_points):
    ...
    '''
    for t_pair in triangle_pairs:

        src_t = t_pair[0]
        dst_t = t_pair[1]

        draw_line(src_pixels, output_size, (255,0,0), src_t[0], src_t[1])
        draw_line(src_pixels, output_size, (255,0,0), src_t[0], src_t[2])
        draw_line(src_pixels, output_size, (255,0,0), src_t[1], src_t[2])
        
        draw_line(dst_pixels, output_size, (255,0,0), dst_t[0], dst_t[1])
        draw_line(dst_pixels, output_size, (255,0,0), dst_t[0], dst_t[2])
        draw_line(dst_pixels, output_size, (255,0,0), dst_t[1], dst_t[2])
        

    output_img = Image.new(src_mode, output_size)
    output_img.putdata(src_pixels)
    output_img.save("src img triangles ploted.jpg")

    output_img = Image.new(dst_mode, output_size)
    output_img.putdata(dst_pixels)
    output_img.save("dst img triangles ploted.jpg")
    '''
    ...
    while t <= 1:
        ...
        for i in now_src_triangle_pairs:
            print i   
        ...
        if count == 10:
            ...
            raw_input()
            mini_count = 0
            for triangle in now_triangles:
                for i in range(len(idx_map)):
                    idx_map[i] = -1
                draw_convex_polygon(triangle, idx_map, output_size, 255)
                output_img = Image.new('L', output_size)
                output_img.putdata(idx_map)
                output_img.save(str(count)+" idx_map "+str(mini_count)+".png")
                mini_count += 1

            tmp_pixels = src_pixels[:]
            for edge in now_edges:
                draw_line(tmp_pixels, output_size, (255,0,0), edge[1], edge[0])

            output_img = Image.new(dst_mode, output_size)
            output_img.putdata(tmp_pixels)
            output_img.save(str(count)+" tmp_pixels.png")
        '''
        ...
        output_img.save(str(count)+" output_pixels "+".png")
        count += 1
