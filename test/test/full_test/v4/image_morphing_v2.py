...
def morph_baby(src_img, dst_img, src_points, dst_points, step):
    ...
    output_pixels = [(0,0,0)]*(output_height*output_width)
    ...
    while phase <= 1:
        ...
        for h in range(output_height):
            for w in range(output_width):
                ...
                if m_idx == -1:
                    output_pixels[h*output_width+w] = (0, 0, 0)
                    continue
                ...
                src_pixel = [(1-phase)*i for i in src_pixel]
                dst_pixel = [phase*i for i in dst_pixel]

                now_pixel = [int(src_pixel[i]+dst_pixel[i]) for i in range(3)]

                output_pixels[h*output_width+w] = tuple(now_pixel)
