...
def morph_baby(src_img, dst_img, src_points, dst_points, step):
    ...
    while t <= 1:
        ...
        dual_matrix_list = [None]*len(now_src_triangle_pairs)

        for i in range(len(now_src_triangle_pairs)):
            dual_matrix = [None, None]
            dual_matrix[0] = get_warping_matrix(now_src_triangle_pairs[i][0], now_src_triangle_pairs[i][1])
            dual_matrix[1] = get_warping_matrix(now_dst_triangle_pairs[i][0], now_dst_triangle_pairs[i][1])
        ...
        for e in now_edges:
            draw_line(tmp_pixels, output_size, (255,0,0), e[0], e[1])
